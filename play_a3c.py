...
def main():
    ...
    env = build_env(args.type, args, render=not args.no_render, treat_life_lost_as_terminal=False, record_screen_dir=args.gif_path, duration=args.duration*60)
    model = A3CLSTM(env.number_of_actions)
    model.load_state_dict(torch.load(os.path.join(args.model_path, args.model_name)))
    model.eval()
    
    model_eval(model, env, dqn_phi, random=False)
